...
app.get('/result/:id2', isLoggedIn, function(req, res) {
    Topic.findById(req.params.id2).populate('mcqs').exec(
        function(err, topic) {
            if (err) {
                ...
                console.log(topic)
                res.render('result', { marks: topic.performance.marks });
            }
        }
    )
});
    ...
    Topic.findById(req.params.id2).populate('mcqs').exec(
        function(err, topic) {
            if (err) {
                ...
                console.log("marks" + Topic.passMarks);
                ...
                topic.save();
                res.render("result.html", { marks: topic.performance.marks })
            }
        }
    )
});
...
app.post('subject/:id/topic', function(req, res) {
    ...
    console.log(pass)
    var newTopic = { topicName: topicName, marks: marks, passMarks: passMarks, time: time };
